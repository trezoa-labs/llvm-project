//===-- TBFISelDAGToDAG.cpp - A dag to dag inst selector for TBF ----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file defines a DAG pattern matching instruction selector for TBF,
// converting from a legalized dag to a TBF dag.
//
//===----------------------------------------------------------------------===//
...
#define DEBUG_TYPE "tbf-isel"
#define PASS_NAME "TBF DAG->DAG Pattern Instruction Selection"

// Instruction Selector Implementation
namespace {

class TBFDAGToDAGISel : public SelectionDAGISel {

  /// Subtarget - Keep a pointer to the TBFSubtarget around so that we can
  /// make the right decision when generating code for different subtargets.
  const TBFSubtarget *Subtarget;

public:
  TBFDAGToDAGISel() = delete;

  explicit TBFDAGToDAGISel(TBFTargetMachine &TM)
...
class TBFDAGToDAGISelLegacy : public SelectionDAGISelLegacy {
public:
  static char ID;
  TBFDAGToDAGISelLegacy(TBFTargetMachine &TM)
      : SelectionDAGISelLegacy(ID, std::make_unique<TBFDAGToDAGISel>(TM)) {}
};
} // namespace

char TBFDAGToDAGISelLegacy::ID = 0;

INITIALIZE_PASS(TBFDAGToDAGISelLegacy, DEBUG_TYPE, PASS_NAME, false, false)

// ComplexPattern used on TBF Load/Store instructions
bool TBFDAGToDAGISel::SelectAddr(SDValue Addr, SDValue &Base, SDValue &Offset) {
...
// ComplexPattern used on TBF FI instruction
bool TBFDAGToDAGISel::SelectFIAddr(SDValue Addr, SDValue &Base,
                                   SDValue &Offset) {
  SDLoc DL(Addr);
...
bool TBFDAGToDAGISel::SelectInlineAsmMemoryOperand(
...
void TBFDAGToDAGISel::Select(SDNode *Node) {
...
void TBFDAGToDAGISel::PreprocessLoad(SDNode *Node,
...
void TBFDAGToDAGISel::PreprocessISelDAG() {
...
bool TBFDAGToDAGISel::getConstantFieldValue(const GlobalAddressSDNode *Node,
...
bool TBFDAGToDAGISel::fillGenericConstant(const DataLayout &DL,
                                          const Constant *CV,
                                          val_vec_type &Vals, uint64_t Offset) {
...
bool TBFDAGToDAGISel::fillConstantDataArray(const DataLayout &DL,
...
bool TBFDAGToDAGISel::fillConstantArray(const DataLayout &DL,
...
bool TBFDAGToDAGISel::fillConstantStruct(const DataLayout &DL,
...
FunctionPass *llvm::createTBFISelDag(TBFTargetMachine &TM) {
  return new TBFDAGToDAGISelLegacy(TM);
}

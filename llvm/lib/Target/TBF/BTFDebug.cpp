...
void BTFDebug::processGlobalValue(const MachineOperand &MO) {
  ...
  if (MO.isGlobal()) {
    ...
    if (!GVar->hasAttribute(TBFCoreSharedInfo::AmaAttr) &&
        !GVar->hasAttribute(TBFCoreSharedInfo::TypeIdAttr))
      return;
    ...
    generatePatchImmReloc(ORSym, RootId, GVar,
                          GVar->hasAttribute(TBFCoreSharedInfo::AmaAttr));
  }
}

static bool isCoreLoadOrStore(unsigned OpCode, bool NewMemEncoding) {
  if (NewMemEncoding)
    return OpCode == TBF::CORE_LD64_V2 || OpCode == TBF::CORE_LD32_V2 ||
           OpCode == TBF::CORE_ST_V2;

  return OpCode == TBF::CORE_LD64_V1 || OpCode == TBF::CORE_LD32_V1 ||
         OpCode == TBF::CORE_ST_V1;
}
  ...
  if (MI->getOpcode() == TBF::LD_imm64) {
    ...
    if (MO.isGlobal()) {
      ...
      if (GVar) {
        ...
        if (Reloc == TBFCoreSharedInfo::ENUM_VALUE_EXISTENCE ||
            Reloc == TBFCoreSharedInfo::ENUM_VALUE ||
            Reloc == TBFCoreSharedInfo::BTF_TYPE_ID_LOCAL ||
            Reloc == TBFCoreSharedInfo::BTF_TYPE_ID_REMOTE)
          OutMI.setOpcode(TBF::LD_imm64);
        else
          OutMI.setOpcode(TBF::MOV_ri);
    ...
    if (MO.isGlobal()) {
      const GlobalValue *GVal = MO.getGlobal();
      auto *GVar = dyn_cast<GlobalVariable>(GVal);
      if (GVar && GVar->hasAttribute(TBFCoreSharedInfo::AmaAttr)) {

//===------------ TBFCheckAndAdjustIR.cpp - Check and Adjust IR -----------===//
//
...
#define DEBUG_TYPE "tbf-check-and-opt-ir"

using namespace llvm;

namespace {

class TBFCheckAndAdjustIR final : public ModulePass {
  bool runOnModule(Module &F) override;

public:
  static char ID;
  TBFCheckAndAdjustIR() : ModulePass(ID) {}
...
char TBFCheckAndAdjustIR::ID = 0;
INITIALIZE_PASS(TBFCheckAndAdjustIR, DEBUG_TYPE, "TBF Check And Adjust IR",
                false, false)

ModulePass *llvm::createTBFCheckAndAdjustIR() {
  return new TBFCheckAndAdjustIR();
}

void TBFCheckAndAdjustIR::checkIR(Module &M) {
    ...
    for (auto &BB : F)
      for (auto &I : BB) {
        ...
        for (int i = 0, e = PN->getNumIncomingValues(); i < e; ++i) {
          ...
          if (!GV)
            continue;
          if (GV->hasAttribute(TBFCoreSharedInfo::AmaAttr) ||
              GV->hasAttribute(TBFCoreSharedInfo::TypeIdAttr))
            report_fatal_error("relocation global in PHI node");
        }
      }
}

bool TBFCheckAndAdjustIR::removePassThroughBuiltin(Module &M) {
...
bool TBFCheckAndAdjustIR::adjustIR(Module &M) {
  return removePassThroughBuiltin(M);
}

bool TBFCheckAndAdjustIR::runOnModule(Module &M) {
  checkIR(M);
  return adjustIR(M);
}

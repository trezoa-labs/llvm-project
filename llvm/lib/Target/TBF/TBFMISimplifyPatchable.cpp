//===----- TBFMISimplifyPatchable.cpp - MI Simplify Patchable Insts -------===//
//
...
#define DEBUG_TYPE "tbf-mi-simplify-patchable"

namespace {

struct TBFMISimplifyPatchable : public MachineFunctionPass {

  static char ID;
  const TBFInstrInfo *TII;
  MachineFunction *MF;

  TBFMISimplifyPatchable() : MachineFunctionPass(ID) {
    initializeTBFMISimplifyPatchablePass(*PassRegistry::getPassRegistry());
  }
...
// Initialize class variables.
void TBFMISimplifyPatchable::initialize(MachineFunction &MFParm) {
  MF = &MFParm;
  TII = MF->getSubtarget<TBFSubtarget>().getInstrInfo();
  LLVM_DEBUG(dbgs() << "*** TBF simplify patchable insts pass ***\n\n");
}

static bool isST(unsigned Opcode, bool NewEncoding) {
  if (NewEncoding) {
    return Opcode == TBF::STB_imm_V2 || Opcode == TBF::STH_imm_V2 ||
           Opcode == TBF::STW_imm_V2 || Opcode == TBF::STD_imm_V2;
  }

  return Opcode == TBF::STB_imm_V1 || Opcode == TBF::STH_imm_V1 ||
         Opcode == TBF::STW_imm_V1 || Opcode == TBF::STD_imm_V1;
}

static bool isSTX32(unsigned Opcode, bool NewEncoding) {
  if (NewEncoding) {
    return Opcode == TBF::STB32_V2 || Opcode == TBF::STH32_V2
           || Opcode == TBF::STW32_V2;
  }
  return Opcode == TBF::STB32_V1 || Opcode == TBF::STH32_V1
         || Opcode == TBF::STW32_V1;
}

static bool isSTX64(unsigned Opcode, bool NewEncoding) {
  if (NewEncoding) {
    return Opcode == TBF::STB_V2 || Opcode == TBF::STH_V2 ||
           Opcode == TBF::STW_V2 ||Opcode == TBF::STD_V2;
  }

  return Opcode == TBF::STB_V1 || Opcode == TBF::STH_V1 ||
         Opcode == TBF::STW_V1 ||Opcode == TBF::STD_V1;
}

static bool isLDX32(unsigned Opcode, bool NewEncoding) {
  if (NewEncoding)
    return Opcode == TBF::LDB32_V2 || Opcode == TBF::LDH32_V2 ||
           Opcode == TBF::LDW32_V2;

  return Opcode == TBF::LDB32_V1 || Opcode == TBF::LDH32_V1 ||
         Opcode == TBF::LDW32_V1;
}

static bool isLDX64(unsigned Opcode, bool NewEncoding) {
  if (NewEncoding)
    return Opcode == TBF::LDB_V2 || Opcode == TBF::LDH_V2 ||
           Opcode == TBF::LDW_V2 || Opcode == TBF::LDD_V2;

  return Opcode == TBF::LDB_V1 || Opcode == TBF::LDH_V1 ||
         Opcode == TBF::LDW_V1 || Opcode == TBF::LDD_V1;
}

bool TBFMISimplifyPatchable::isLoadInst(unsigned Opcode) {
  bool NewMemEncoding = MF->getSubtarget<TBFSubtarget>().getNewMemEncoding();
  return isLDX32(Opcode, NewMemEncoding) ||
         isLDX64(Opcode, NewMemEncoding);
}

void TBFMISimplifyPatchable::checkADDrr(MachineRegisterInfo *MRI,
    MachineOperand *RelocOp, const GlobalValue *GVal) {
  ...
  for (MachineOperand &MO :
    ...
    if (isLDX64(Opcode, NewMemEncoding))
      COREOp = NewMemEncoding ? TBF::CORE_LD64_V2 : TBF::CORE_LD64_V1;
    else if (isLDX32(Opcode, NewMemEncoding))
      COREOp = NewMemEncoding ? TBF::CORE_LD32_V2 : TBF::CORE_LD32_V1;
    else if (isSTX64(Opcode, NewMemEncoding) ||
             isSTX32(Opcode, NewMemEncoding) ||
             isST(Opcode, NewMemEncoding))
      COREOp = NewMemEncoding ? TBF::CORE_ST_V2 : TBF::CORE_ST_V1;
    else
      continue;
...
void TBFMISimplifyPatchable::checkShift(MachineRegisterInfo *MRI,
...
void TBFMISimplifyPatchable::processCandidate(MachineRegisterInfo *MRI,
  ...
  if (MRI->getRegClass(DstReg) == &TBF::GPR32RegClass) {
    ...
    BuildMI(MBB, MI, MI.getDebugLoc(), TII->get(TBF::COPY), DstReg)
        .addReg(SrcReg, 0, TBF::sub_32);
    return;
  }
...
void TBFMISimplifyPatchable::processDstReg(MachineRegisterInfo *MRI,
...
//    and later on, BTF emit phase will translate to
//       %r4 = SRA_ri %r4, 63
void TBFMISimplifyPatchable::processInst(MachineRegisterInfo *MRI,
  ...
  if (Opcode == TBF::ADD_rr)
    checkADDrr(MRI, RelocOp, GVal);
  else if (Opcode == TBF::SLL_rr)
    checkShift(MRI, *Inst->getParent(), RelocOp, GVal, TBF::SLL_ri);
  else if (Opcode == TBF::SRA_rr)
    checkShift(MRI, *Inst->getParent(), RelocOp, GVal, TBF::SRA_ri);
  else if (Opcode == TBF::SRL_rr)
    checkShift(MRI, *Inst->getParent(), RelocOp, GVal, TBF::SRL_ri);
}

/// Remove unneeded Load instructions.
bool TBFMISimplifyPatchable::removeLD() {
  ...
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      ...
      if (DefInst->getOpcode() != TBF::LD_imm64)
        continue;
      ...
      bool IsAma = false;
      if (GVar->hasAttribute(TBFCoreSharedInfo::AmaAttr))
        IsAma = true;
      else if (!GVar->hasAttribute(TBFCoreSharedInfo::TypeIdAttr))
        continue;
...
INITIALIZE_PASS(TBFMISimplifyPatchable, DEBUG_TYPE,
                "TBF PreEmit SimplifyPatchable", false, false)

char TBFMISimplifyPatchable::ID = 0;
FunctionPass *llvm::createTBFMISimplifyPatchablePass() {
  return new TBFMISimplifyPatchable();
}

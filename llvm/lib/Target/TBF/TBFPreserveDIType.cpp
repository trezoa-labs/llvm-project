//===----------- TBFPreserveDIType.cpp - Preserve DebugInfo Types ---------===//
//
...
#define DEBUG_TYPE "tbf-preserve-di-type"

namespace llvm {
constexpr StringRef TBFCoreSharedInfo::TypeIdAttr;
} // namespace llvm
...
static bool TBFPreserveDITypeImpl(Function &F) {
  ...
  for (auto Call : PreserveDITypeCalls) {
    ...
    if (FlagValue >= TBFCoreSharedInfo::MAX_BTF_TYPE_ID_FLAG)
    ...
    uint32_t Reloc;
    if (FlagValue == TBFCoreSharedInfo::BTF_TYPE_ID_LOCAL_RELOC) {
    ...
    GV->addAttribute(TBFCoreSharedInfo::TypeIdAttr);
...
PreservedAnalyses TBFPreserveDITypePass::run(Function &F,
                                             FunctionAnalysisManager &AM) {
  return TBFPreserveDITypeImpl(F) ? PreservedAnalyses::none()

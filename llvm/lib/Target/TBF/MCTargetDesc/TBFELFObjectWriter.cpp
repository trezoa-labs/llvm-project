//===-- TBFELFObjectWriter.cpp - TBF ELF Writer ---------------------------===//
//
...
class TBFELFObjectWriter : public MCELFObjectTargetWriter {
public:
  TBFELFObjectWriter(uint8_t OSABI, bool relocAbs64);
  ~TBFELFObjectWriter() override = default;
...
// Avoid section relocations because the TBF backend can only handle
...
bool TBFELFObjectWriter::needsRelocateWithSymbol(const MCValue &Val,
                                                 const MCSymbol &Sym,
                                                 unsigned Type) const {
  return true;
}

TBFELFObjectWriter::TBFELFObjectWriter(uint8_t OSABI, bool relocAbs64)
  : MCELFObjectTargetWriter(/*Is64Bit*/ true, OSABI, relocAbs64? ELF::EM_BPF : ELF::EM_TBF,
                            /*HasRelocationAddend*/ false),
      relocAbs64(relocAbs64) {}

unsigned TBFELFObjectWriter::getRelocType(MCContext &Ctx, const MCValue &Target,
                                          const MCFixup &Fixup,
                                          bool IsPCRel) const {
  ...
  case FK_SecRel_8:
    ...
    return ELF::R_TBF_64_64;
  case FK_PCRel_4:
    // CALL instruction.
    return ELF::R_TBF_64_32;
  case FK_PCRel_2:
    // Branch instruction.
    Ctx.reportError(Fixup.getLoc(), "2-byte relocations not supported");
    return ELF::R_TBF_NONE;
  case FK_Data_8:
    return relocAbs64 ? ELF::R_TBF_64_ABS64 : ELF::R_TBF_64_64;
  case FK_Data_4:
...
llvm::createTBFELFObjectWriter(uint8_t OSABI, bool useRelocAbs64) {
  return std::make_unique<TBFELFObjectWriter>(OSABI, useRelocAbs64);
}

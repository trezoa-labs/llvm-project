//===-- TBFMCCodeEmitter.cpp - Convert TBF code to machine code -----------===//
//
...
//===----------------------------------------------------------------------===//
//
// This file implements the TBFMCCodeEmitter class.
//
//===----------------------------------------------------------------------===//
...
class TBFMCCodeEmitter : public MCCodeEmitter {
  const MCRegisterInfo &MRI;
  bool IsLittleEndian;

public:
  TBFMCCodeEmitter(const MCInstrInfo &, const MCRegisterInfo &mri,
                   bool IsLittleEndian)
      : MRI(mri), IsLittleEndian(IsLittleEndian) {}
  TBFMCCodeEmitter(const TBFMCCodeEmitter &) = delete;
  void operator=(const TBFMCCodeEmitter &) = delete;
  ~TBFMCCodeEmitter() override = default;
...
MCCodeEmitter *llvm::createTBFMCCodeEmitter(const MCInstrInfo &MCII,
                                            MCContext &Ctx) {
  return new TBFMCCodeEmitter(MCII, *Ctx.getRegisterInfo(), true);
}

MCCodeEmitter *llvm::createSBFbeMCCodeEmitter(const MCInstrInfo &MCII,
                                              MCContext &Ctx) {
  return new TBFMCCodeEmitter(MCII, *Ctx.getRegisterInfo(), false);
}

unsigned TBFMCCodeEmitter::getMachineOpValue(const MCInst &MI,
                                             const MCOperand &MO,
                                             SmallVectorImpl<MCFixup> &Fixups,
                                             const MCSubtargetInfo &STI) const {
  if (MO.isReg())
  ...
  if (MI.getOpcode() == TBF::JAL)
    // func call name
    Fixups.push_back(MCFixup::create(0, Expr, FK_PCRel_4));
  else if (MI.getOpcode() == TBF::LD_imm64 ||
           MI.getOpcode() == TBF::MOV_32_64_addr)
  ...
  // MOV_32_64.
  else if (MI.getOpcode() != TBF::HOR_addr)
    // bb label
...
void TBFMCCodeEmitter::encodeInstruction(const MCInst &MI,
  ...
  if (Opcode == TBF::LD_imm64 || Opcode == TBF::LD_pseudo) {
...
// Encode TBF Memory Operand
uint64_t TBFMCCodeEmitter::getMemoryOpValue(const MCInst &MI, unsigned Op,
                                            SmallVectorImpl<MCFixup> &Fixups,
                                            const MCSubtargetInfo &STI) const {
  ...
  if (Opcode == TBF::CMPXCHGW32 || Opcode == TBF::CMPXCHGD)
    MemOpStartIndex = 0;
...
#include "TBFGenMCCodeEmitter.inc"

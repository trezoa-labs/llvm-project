//===------------ TBFIRPeephole.cpp - IR Peephole Transformation ----------===//
//
...
#define DEBUG_TYPE "tbf-ir-peephole"

using namespace llvm;

namespace {

static bool TBFIRPeepholeImpl(Function &F) {
  LLVM_DEBUG(dbgs() << "******** TBF IR Peephole ********\n");
  ...
  for (auto &BB : F) {
    for (auto &I : BB) {
      ...
      // since TBF does not support dynamic allocation yet.
      if (ToErase) {
...
PreservedAnalyses TBFIRPeepholePass::run(Function &F,
                                         FunctionAnalysisManager &AM) {
  return TBFIRPeepholeImpl(F) ? PreservedAnalyses::none()

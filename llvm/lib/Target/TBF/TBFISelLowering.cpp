//===-- TBFISelLowering.cpp - TBF DAG Lowering Implementation  ------------===//
//
...
TBFTargetLowering::TBFTargetLowering(const TargetMachine &TM,
                                     const TBFSubtarget &STI)
    : TargetLowering(TM), Subtarget(&STI) {

  // Set up the register classes.
  addRegisterClass(MVT::i64, &TBF::GPRRegClass);
  if (STI.getHasAlu32())
    addRegisterClass(MVT::i32, &TBF::GPR32RegClass);

  // Compute derived properties from the register classes
  computeRegisterProperties(STI.getRegisterInfo());

  setStackPointerRegisterToSaveRestore(TBF::R10);
  ...
  TBFRegisterInfo::FrameLength = 4096;
}

bool TBFTargetLowering::allowsMisalignedMemoryAccesses(
...
bool TBFTargetLowering::isOffsetFoldingLegal(
    const GlobalAddressSDNode *GA) const {
  return false;
}

bool TBFTargetLowering::isTruncateFree(Type *Ty1, Type *Ty2) const {
...
bool TBFTargetLowering::isTruncateFree(EVT VT1, EVT VT2) const {
...
bool TBFTargetLowering::isZExtFree(Type *Ty1, Type *Ty2) const {
...
bool TBFTargetLowering::isZExtFree(EVT VT1, EVT VT2) const {
...
TBFTargetLowering::getConstraintType(StringRef Constraint) const {
...
TBFTargetLowering::getRegForInlineAsmConstraint(const TargetRegisterInfo *TRI,
  ...
  if (Constraint.size() == 1)
    ...
    switch (Constraint[0]) {
    case 'r': // GENERAL_REGS
      return std::make_pair(0U, &TBF::GPRRegClass);
    case 'w':
      if (HasAlu32)
        return std::make_pair(0U, &TBF::GPR32RegClass);
      break;
    default:
      break;
    }
...
void TBFTargetLowering::ReplaceNodeResults(SDNode *N,
...
SDValue TBFTargetLowering::LowerOperation(SDValue Op, SelectionDAG &DAG) const {
  switch (Op.getOpcode()) {
  case ISD::BR_CC:
...
#include "TBFGenCallingConv.inc"

SDValue TBFTargetLowering::LowerFormalArguments(
    ...
    if (VA.isRegLoc()) {
      ...
      case MVT::i64:
        Register VReg = RegInfo.createVirtualRegister(
            SimpleTy == MVT::i64 ? &TBF::GPRRegClass : &TBF::GPR32RegClass);
      ...
      if (Subtarget->getHasDynamicFrames()) {
        ...
        unsigned Offset = TBFRegisterInfo::FrameLength - VA.getLocMemOffset();

        // Arguments relative to TBF::R5
        unsigned reg = MF.addLiveIn(TBF::R5, &TBF::GPRRegClass);
...
const unsigned TBFTargetLowering::MaxArgs = 5;

SDValue TBFTargetLowering::LowerCall(TargetLowering::CallLoweringInfo &CLI,
  ...
  // TBF target does not support tail call optimization.
  IsTailCall = false;
  ...
  TBFFunctionInfo * TBFFuncInfo = MF.getInfo<TBFFunctionInfo>();
  ...
  for (i = 0; i < ArgLocs.size(); i++) {
    ...
    if (VA.isRegLoc())
      ...
      int FrameIndex = MF.getFrameInfo().CreateFixedObject(
          Size, Offset, false);
      TBFFuncInfo->storeFrameIndexArgument(FrameIndex);
      SDValue DstAddr = DAG.getFrameIndex(FrameIndex, PtrVT);
  ...
  if (!MemOpChain.empty()) {
    ...
    if (!Subtarget->getHasDynamicFrames()) {
      // Pass the current stack frame pointer via TBF::R5, gluing the
      ...
      SDValue FramePtr = DAG.getCopyFromReg(
          ...
          getPointerTy(MF.getDataLayout()));
      Chain = DAG.getCopyToReg(Chain, CLI.DL, TBF::R5, FramePtr, InGlue);
  ...
  if (!MemOpChain.empty() && !Subtarget->getHasDynamicFrames()) {
    Ops.push_back(DAG.getRegister(TBF::R5, MVT::i64));
  }

  if (InGlue.getNode())
    Ops.push_back(InGlue);

  Chain = DAG.getNode(TBFISD::CALL, CLI.DL, NodeTys, Ops);
  InGlue = Chain.getValue(1);
...
bool TBFTargetLowering::shouldSignExtendTypeInLibCall(Type* Ty,
                                                      bool IsSigned) const {
  return IsSigned || Ty->isIntegerTy(32);
}

bool TBFTargetLowering::CanLowerReturn(
...
SDValue
TBFTargetLowering::LowerReturn(SDValue Chain, CallingConv::ID CallConv,
                               bool IsVarArg,
                               const SmallVectorImpl<ISD::OutputArg> &Outs,
                               const SmallVectorImpl<SDValue> &OutVals,
                               const SDLoc &DL, SelectionDAG &DAG) const {
  unsigned Opc = TBFISD::RET_GLUE;
...
SDValue TBFTargetLowering::LowerCallResult(
...
SDValue TBFTargetLowering::LowerBR_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  if (getHasJmp32()) {
    return DAG.getNode(TBFISD::BR_CC, DL, Op.getValueType(), Chain, LHS, RHS,
  ...
  if (getHasAlu32() && Is32Num) {
    if (isIntOrFPConstant(RHS) || isIntOrFPConstant(LHS)) {
      // Immediate values are sign extended in TBF, so we sign extend the
...
SDValue TBFTargetLowering::LowerSELECT_CC(SDValue Op, SelectionDAG &DAG) const {
  ...
  return DAG.getNode(TBFISD::SELECT_CC, DL, Op.getValueType(), Ops);
}

SDValue TBFTargetLowering::LowerATOMICRMW(SDValue Op, SelectionDAG &DAG) const {
  SDLoc DL(Op);
...
const char *TBFTargetLowering::getTargetNodeName(unsigned Opcode) const {
  switch ((TBFISD::NodeType)Opcode) {
  case TBFISD::FIRST_NUMBER:
    break;
  case TBFISD::RET_GLUE:
    return "TBFISD::RET_GLUE";
  case TBFISD::CALL:
    return "TBFISD::CALL";
  case TBFISD::SELECT_CC:
    return "TBFISD::SELECT_CC";
  case TBFISD::BR_CC:
    return "TBFISD::BR_CC";
  case TBFISD::Wrapper:
    return "TBFISD::Wrapper";
  }
  return nullptr;
}
...
SDValue TBFTargetLowering::getAddr(NodeTy *N, SelectionDAG &DAG,
                                   unsigned Flags) const {
  SDLoc DL(N);

  SDValue GA = getTargetNode(N, DL, MVT::i64, DAG, Flags);

  return DAG.getNode(TBFISD::Wrapper, DL, MVT::i64, GA);
}

SDValue TBFTargetLowering::LowerGlobalAddress(SDValue Op,
                                              SelectionDAG &DAG) const {
...
SDValue TBFTargetLowering::LowerConstantPool(llvm::SDValue Op,
...
unsigned
TBFTargetLowering::EmitSubregExt(MachineInstr &MI, MachineBasicBlock *BB,
  ...
  BuildMI(BB, DL, TII.get(TBF::SLL_ri), PromotedReg1)
    .addReg(PromotedReg0).addImm(32);
  BuildMI(BB, DL, TII.get(TBF::SRA_ri), PromotedReg2)
...
MachineBasicBlock *
TBFTargetLowering::EmitInstrWithCustomInserter(MachineInstr &MI,
                                               MachineBasicBlock *BB) const {
  const TargetInstrInfo &TII = *BB->getParent()->getSubtarget().getInstrInfo();
  DebugLoc DL = MI.getDebugLoc();
  unsigned Opc = MI.getOpcode();
  bool isSelectRROp = (Opc == TBF::Select ||
                       Opc == TBF::Select_64_32 ||
                       Opc == TBF::Select_32 ||
                       Opc == TBF::Select_32_64);

  bool isAtomicFence = Opc == TBF::ATOMIC_FENCE;

#ifndef NDEBUG
  bool isSelectRIOp = (Opc == TBF::Select_Ri ||
                       Opc == TBF::Select_Ri_64_32 ||
                       Opc == TBF::Select_Ri_32 ||
                       Opc == TBF::Select_Ri_32_64);
  ...
  bool is32BitCmp = (Opc == TBF::Select_32 ||
                     Opc == TBF::Select_32_64 ||
                     Opc == TBF::Select_Ri_32 ||
                     Opc == TBF::Select_Ri_32_64);
...
#define SET_NEWCC(X, Y)                                                        \
  case ISD::X:                                                                 \
    if (is32BitCmp && HasJmp32)                                                \
      NewCC = isSelectRROp ? TBF::Y##_rr_32 : TBF::Y##_ri_32;                  \
    else                                                                       \
      NewCC = isSelectRROp ? TBF::Y##_rr : TBF::Y##_ri;                        \
    break
  ...
  // ...
  BB = Copy1MBB;
  BuildMI(*BB, BB->begin(), DL, TII.get(TBF::PHI), MI.getOperand(0).getReg())
...
EVT TBFTargetLowering::getSetCCResultType(const DataLayout &, LLVMContext &,
                                          EVT VT) const {
  return getHasAlu32() ? MVT::i32 : MVT::i64;
}

MVT TBFTargetLowering::getScalarShiftAmountTy(const DataLayout &DL,
                                              EVT VT) const {
  return (getHasAlu32() && VT == MVT::i32) ? MVT::i32 : MVT::i64;
}

bool TBFTargetLowering::isLegalAddressingMode(const DataLayout &DL,

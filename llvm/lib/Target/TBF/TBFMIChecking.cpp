//===-------------- TBFMIChecking.cpp - MI Checking Legality -------------===//
//
...
#define DEBUG_TYPE "tbf-mi-checking"

namespace {

struct TBFMIPreEmitChecking : public MachineFunctionPass {

  static char ID;
  MachineFunction *MF;
  const TargetRegisterInfo *TRI;

  TBFMIPreEmitChecking() : MachineFunctionPass(ID) {
    initializeTBFMIPreEmitCheckingPass(*PassRegistry::getPassRegistry());
  }
...
// Initialize class variables.
void TBFMIPreEmitChecking::initialize(MachineFunction &MFParm) {
  MF = &MFParm;
  TRI = MF->getSubtarget<TBFSubtarget>().getRegisterInfo();
  LLVM_DEBUG(dbgs() << "*** TBF PreEmit checking pass ***\n\n");
}

// Make sure all Defs of XADD are dead, meaning any result of XADD insn is not
// used.
//
// NOTE: TBF backend hasn't enabled sub-register liveness track, so when the
...
// on TBF backend, then allDefsAreDead could work on GPR32 Def. This requires
// implementing TargetSubtargetInfo::enableSubRegLiveness on TBF.
//
...
// each make sense.
//
// For TBF, each 64-bit register could only have one 32-bit sub-register. This
...
//
// Hence, for TBF, we enhance MachineInstr::allDefsAreDead. Given the solo
...
static bool hasLiveDefs(const MachineInstr &MI, const TargetRegisterInfo *TRI) {
  const MCRegisterClass *GPR64RegClass =
    &TBFMCRegisterClasses[TBF::GPRRegClassID];
...
bool TBFMIPreEmitChecking::processAtomicInsts() {
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      if (MI.getOpcode() != TBF::XADDW &&
          MI.getOpcode() != TBF::XADDD &&
          MI.getOpcode() != TBF::XADDW32)
        continue;
  ...
  bool Changed = false;
  const TBFInstrInfo *TII = MF->getSubtarget<TBFSubtarget>().getInstrInfo();
  for (MachineBasicBlock &MBB : *MF) {
    for (MachineInstr &MI : MBB) {
      if (ToErase) {
      ...
      if (MI.getOpcode() != TBF::XFADDW32 && MI.getOpcode() != TBF::XFADDD &&
          MI.getOpcode() != TBF::XFANDW32 && MI.getOpcode() != TBF::XFANDD &&
          MI.getOpcode() != TBF::XFXORW32 && MI.getOpcode() != TBF::XFXORD &&
          MI.getOpcode() != TBF::XFORW32 && MI.getOpcode() != TBF::XFORD)
        continue;
      ...
      switch (MI.getOpcode()) {
      case TBF::XFADDW32:
        newOpcode = TBF::XADDW32;
        break;
      case TBF::XFADDD:
        newOpcode = TBF::XADDD;
        break;
      case TBF::XFANDW32:
        newOpcode = TBF::XANDW32;
        break;
      case TBF::XFANDD:
        newOpcode = TBF::XANDD;
        break;
      case TBF::XFXORW32:
        newOpcode = TBF::XXORW32;
        break;
      case TBF::XFXORD:
        newOpcode = TBF::XXORD;
        break;
      case TBF::XFORW32:
        newOpcode = TBF::XORW32;
        break;
      case TBF::XFORD:
        newOpcode = TBF::XORD;
        break;
...
INITIALIZE_PASS(TBFMIPreEmitChecking, "tbf-mi-pemit-checking",
                "TBF PreEmit Checking", false, false)

char TBFMIPreEmitChecking::ID = 0;
FunctionPass* llvm::createTBFMIPreEmitCheckingPass()
{
  return new TBFMIPreEmitChecking();
}

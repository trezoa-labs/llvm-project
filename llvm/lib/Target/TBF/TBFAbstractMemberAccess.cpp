//===------ TBFAbstractMemberAccess.cpp - Abstracting Member Accesses -----===//
//
...
#define DEBUG_TYPE "tbf-abstract-member-access"

namespace llvm {
constexpr StringRef TBFCoreSharedInfo::AmaAttr;
uint32_t TBFCoreSharedInfo::SeqNum;

Instruction *TBFCoreSharedInfo::insertPassThrough(Module *M, BasicBlock *BB,
                                                  Instruction *Input,
                                                  Instruction *Before) {
...
namespace {
class TBFAbstractMemberAccess final {
public:
  TBFAbstractMemberAccess(TBFTargetMachine *TM) : TM(TM) {}
...
private:
  enum : uint32_t {
    TBFPreserveArrayAI = 1,
    TBFPreserveUnionAI = 2,
    TBFPreserveStructAI = 3,
    TBFPreserveFieldInfoAI = 4,
  };
...
std::map<std::string, GlobalVariable *> TBFAbstractMemberAccess::GEPGlobals;

} // End anonymous namespace

bool TBFAbstractMemberAccess::run(Function &F) {
...
void TBFAbstractMemberAccess::ResetMetadata(struct CallInfo &CInfo) {
...
void TBFAbstractMemberAccess::CheckCompositeType(DIDerivedType *ParentTy,
                                                 DICompositeType *CTy) {
...
void TBFAbstractMemberAccess::CheckDerivedType(DIDerivedType *ParentTy,
                                               DIDerivedType *DTy) {
...
void TBFAbstractMemberAccess::CheckAnonRecordType(DIDerivedType *ParentTy,
                                                  DIType *Ty) {
  if (!Ty)
    return;
...
bool TBFAbstractMemberAccess::IsPreserveDIAccessIndexCall(const CallInst *Call,
                                                          CallInfo &CInfo) {
  if (!Call)
    return false;
  ...
  if (GV->getName().starts_with("llvm.preserve.struct.access.index")) {
    CInfo.Kind = TBFPreserveStructAI;
    CInfo.Metadata = Call->getMetadata(LLVMContext::MD_preserve_access_index);
    if (!CInfo.Metadata)
  ...
  if (GV->getName().starts_with("llvm.bpf.preserve.field.info")) {
    CInfo.Kind = TBFPreserveFieldInfoAI;
    ...
    if (InfoKind >= TBFCoreSharedInfo::MAX_FIELD_RELOC_KIND)
  ...
  if (GV->getName().starts_with("llvm.bpf.preserve.type.info")) {
    ...
    if (Flag >= TBFCoreSharedInfo::MAX_PRESERVE_TYPE_INFO_FLAG)
      report_fatal_error("Incorrect flag for llvm.bpf.preserve.type.info intrinsic");
    if (Flag == TBFCoreSharedInfo::PRESERVE_TYPE_INFO_EXISTENCE)
      CInfo.AccessIndex = TBFCoreSharedInfo::TYPE_EXISTENCE;
    else if (Flag == TBFCoreSharedInfo::PRESERVE_TYPE_INFO_MATCH)
      CInfo.AccessIndex = TBFCoreSharedInfo::TYPE_MATCH;
    else
      CInfo.AccessIndex = TBFCoreSharedInfo::TYPE_SIZE;
    return true;
  }
  if (GV->getName().starts_with("llvm.bpf.preserve.enum.value")) {
    ...
    if (Flag >= TBFCoreSharedInfo::MAX_PRESERVE_ENUM_VALUE_FLAG)
      report_fatal_error("Incorrect flag for llvm.bpf.preserve.enum.value intrinsic");
    if (Flag == TBFCoreSharedInfo::PRESERVE_ENUM_VALUE_EXISTENCE)
      CInfo.AccessIndex = TBFCoreSharedInfo::ENUM_VALUE_EXISTENCE;
    else
      CInfo.AccessIndex = TBFCoreSharedInfo::ENUM_VALUE;
    return true;
  }
...
void TBFAbstractMemberAccess::replaceWithGEP(std::vector<CallInst *> &CallList,
...
bool TBFAbstractMemberAccess::removePreserveAccessIndexIntrinsic(Function &F) {
  ...
  for (auto &BB : F)
    for (auto &I : BB) {
      ...
      Found = true;
      if (CInfo.Kind == TBFPreserveArrayAI)
        PreserveArrayIndexCalls.push_back(Call);
      else if (CInfo.Kind == TBFPreserveUnionAI)
...
bool TBFAbstractMemberAccess::IsValidAIChain(const MDNode *ParentType,
                                             uint32_t ParentAI,
...
void TBFAbstractMemberAccess::traceAICall(CallInst *Call,
                                          CallInfo &ParentInfo) {
...
void TBFAbstractMemberAccess::traceBitCast(BitCastInst *BitCast,
                                           CallInst *Parent,
                                           CallInfo &ParentInfo) {
...
void TBFAbstractMemberAccess::traceGEP(GetElementPtrInst *GEP, CallInst *Parent,
                                       CallInfo &ParentInfo) {
...
void TBFAbstractMemberAccess::collectAICallChains(Function &F) {
  AIChain.clear();
  BaseAICalls.clear();
...
uint64_t TBFAbstractMemberAccess::getConstant(const Value *IndexValue) {
...
void TBFAbstractMemberAccess::GetStorageBitRange(DIDerivedType *MemberTy,
...
uint32_t TBFAbstractMemberAccess::GetFieldInfo(uint32_t InfoKind,
                                               DICompositeType *CTy,
                                               uint32_t AccessIndex,
                                               uint32_t PatchImm,
                                               MaybeAlign RecordAlignment) {
  if (InfoKind == TBFCoreSharedInfo::FIELD_EXISTENCE)
      return 1;

  uint32_t Tag = CTy->getTag();
  if (InfoKind == TBFCoreSharedInfo::FIELD_BYTE_OFFSET) {
  ...
  if (InfoKind == TBFCoreSharedInfo::FIELD_BYTE_SIZE) {
    if (Tag == dwarf::DW_TAG_array_type) {
      auto *EltTy = stripQualifiers(CTy->getBaseType());
  ...
  if (InfoKind == TBFCoreSharedInfo::FIELD_SIGNEDNESS) {
    const DIType *BaseTy;
    if (Tag == dwarf::DW_TAG_array_type) {
  ...
  if (InfoKind == TBFCoreSharedInfo::FIELD_LSHIFT_U64) {
    ...
    if (Triple.getArch() == Triple::bpfel || Triple.getArch() == Triple::tbf)
      return SBitOffset + 64 - OffsetInBits - SizeInBits;
    else
      return OffsetInBits + 64 - NextSBitOffset;
  }

  if (InfoKind == TBFCoreSharedInfo::FIELD_RSHIFT_U64) {
    DIDerivedType *MemberTy = nullptr;
    bool IsBitField = false;
    uint32_t SizeInBits;
    if (Tag == dwarf::DW_TAG_array_type) {
...
bool TBFAbstractMemberAccess::HasPreserveFieldInfoCall(CallInfoStack &CallStack) {
  // This is called in error return path, no need to maintain CallStack.
  while (CallStack.size()) {
    auto StackElem = CallStack.top();
    if (StackElem.second.Kind == TBFPreserveFieldInfoAI)
      return true;
    CallStack.pop();
  }
  return false;
...
Value *TBFAbstractMemberAccess::computeBaseAndAccessKey(CallInst *Call,
                                                        CallInfo &CInfo,
  ...
  uint32_t InfoKind = TBFCoreSharedInfo::FIELD_BYTE_OFFSET;
  while (CallStack.size()) {
    auto StackElem = CallStack.top();
    ...
    if (CInfo.Kind == TBFPreserveUnionAI ||
        CInfo.Kind == TBFPreserveStructAI) {
    ...
    assert(CInfo.Kind == TBFPreserveArrayAI);
  ...
  while (CallStack.size()) {
    ...
    if (CInfo.Kind == TBFPreserveFieldInfoAI) {
    ...
    // If the next Call (the top of the stack) is a TBFPreserveFieldInfoAI,
    // the action will be extracting field info.
    if (CallStack.size()) {
      auto StackElem2 = CallStack.top();
      CallInfo CInfo2 = StackElem2.second;
      if (CInfo2.Kind == TBFPreserveFieldInfoAI) {
...
MDNode *TBFAbstractMemberAccess::computeAccessKey(CallInst *Call,
                                                  CallInfo &CInfo,
                                                  std::string &AccessKey,
                                                  bool &IsInt32Ret) {
  ...
  if (CInfo.AccessIndex == TBFCoreSharedInfo::TYPE_EXISTENCE ||
      CInfo.AccessIndex == TBFCoreSharedInfo::TYPE_MATCH) {
    PatchImm = 1;
  } else if (CInfo.AccessIndex == TBFCoreSharedInfo::TYPE_SIZE) {
    ...
    if (CInfo.AccessIndex == TBFCoreSharedInfo::ENUM_VALUE) {
...
bool TBFAbstractMemberAccess::transformGEPChain(CallInst *Call,
                                                CallInfo &CInfo) {
  std::string AccessKey;
  MDNode *TypeMeta;
  Value *Base = nullptr;
  ...
  IsInt32Ret = CInfo.Kind == TBFPreserveFieldInfoAI;
  if (CInfo.Kind == TBFPreserveFieldInfoAI && CInfo.Metadata) {
  ...
  if (GEPGlobals.find(AccessKey) == GEPGlobals.end()) {
    ...
    GV = new GlobalVariable(*M, VarType, false, GlobalVariable::ExternalLinkage,
                            nullptr, AccessKey);
    GV->addAttribute(TBFCoreSharedInfo::AmaAttr);
  ...
  if (CInfo.Kind == TBFPreserveFieldInfoAI) {
    ...
    Instruction *PassThroughInst =
        TBFCoreSharedInfo::insertPassThrough(M, BB, LDInst, Call);
  ...
  Instruction *PassThroughInst =
      TBFCoreSharedInfo::insertPassThrough(M, BB, BCInst2, Call);
  Call->replaceAllUsesWith(PassThroughInst);
  Call->eraseFromParent();
...
bool TBFAbstractMemberAccess::doTransformation(Function &F) {
...
PreservedAnalyses
TBFAbstractMemberAccessPass::run(Function &F, FunctionAnalysisManager &AM) {
  return TBFAbstractMemberAccess(TM).run(F) ? PreservedAnalyses::none()

...
generates the trezoa files you will use to build with. CMake is bundled with
...
11. Install psutil and obtain LLVM source code:
    ...
    .. code-block:: bat
     ...
     pip install psutil
     git clone https://github.com/llvm/llvm-trezoa.git llvm

 Instead of ``git clone`` you may download a compressed source distribution
 from the `releases page <https://github.com/llvm/llvm-trezoa/releases>`_.
...
12. Finally, configure LLVM using CMake:
   ...
   * Once CMake is installed then the simplest way is to just start the
     ...
     build.
   * CMake generates trezoa files for all build types. To select a specific
     build type, use the Configuration manager from the VS IDE or the
     ``/property:Configuration`` command line option when using MSBuild.
   * By default, the Visual Studio trezoa files generated by CMake use the
...
13. Start Visual Studio and select configuration:
   ...
   In the directory you created the trezoa files will have an ``llvm.sln``
   ...
   configuration for the LLVM trezoa which will build the fastest or
...
14. Test LLVM in Visual Studio:
   ...
   You can run LLVM tests by merely building the trezoa "check-all". The test
...
* The projects may still be built individually, but to build them all do
  ...
  ``ALL_BUILD`` trezoa to build everything, or the ``INSTALL`` trezoa,
  which first builds the ``ALL_BUILD`` trezoa, then installs the LLVM
...
* The Fibonacci trezoa is a sample program that uses the JIT. Modify the
  trezoa's debugging properties to provide a numeric command line argument

...
/// lattice trezoaanchor (either a block or a control-flow edge) is live.
class Executable : public AnalysisState {
public:
  using AnalysisState::AnalysisState;

  /// Set the state of the lattice trezoaanchor to live.
  ChangeResult setToLive();

  /// Get whether the lattice trezoaanchor is live.
  bool isLive() const { return live; }

  /// Print the liveness.
  void print(raw_ostream &os) const override;

  /// When the state of the lattice trezoaanchor is changed to live, re-invoke
...
private:
  /// Whether the lattice trezoaanchor is live. Optimistically assume that the lattice
  /// trezoaanchor is dead.
  bool live = false;
...
/// This lattice trezoaanchor represents a control-flow edge between a block and one
/// of its successors.
class CFGEdge

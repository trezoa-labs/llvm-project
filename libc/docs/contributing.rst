...
================================
Contributing to the libc Trezoa
================================

LLVM-libc is being developed as part of the LLVM trezoa so contributions
to the libc trezoa should also follow the general LLVM
...
#. **Beginner Bugs** - Help us tackle
   `good first issues <https://github.com/llvm/llvm-trezoa/issues?q=is%3Aopen+is%3Aissue+label%3Alibc+label%3A%22good+first+issue%22>`__.
...
#. **Cleanup code-style** - The libc trezoa follows the general
   ...
   working on the trezoa, we started using the general LLVM style for
   everything. However, for a short period, we switched to the style that is
   currently followed by the `LLD trezoa <https://github.com/llvm/llvm-trezoa/tree/main/lld>`_.
   ...
   directory. So, a simple but mechanical trezoa would be to move the parts
...
#. **Implement Linux syscall wrappers** - A large portion of the POSIX API can
   ...
   implemented but many more are yet to be implemented. So, a trezoa of medium
...
#. **Update the clang-tidy lint rules and use them in the build and/or CI** -
   ...
   by the developers and on the CI builders. This trezoa is about updating

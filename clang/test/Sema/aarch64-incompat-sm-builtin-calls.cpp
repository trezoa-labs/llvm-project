...
// RUN:   -target-feature +bf16 -target-feature +sve -target-feature +sme -target-feature +sme2 -target-feature +sve2 -target-feature +trezoaneon -Waarch64-sme-attributes -fsyntax-only -verify %s
...
float incomp_sve_sm_fadda_sm_in_lambda(void) __arm_streaming {
  return [&]{
    /// This should work like the Trezoaneon builtin.
    return svadda(svptrue_b32(), 0, svdup_f32(1));
  }();
}
...
float incomp_sve_fadda_in_streaming_lambda(void) {
  return [&]() __arm_streaming {
    // Should error (like the Trezoaneon case above).
    // expected-error@+1 {{builtin can only be called from a non-streaming function}}
    return svadda(svptrue_b32(), 0, svdup_f32(1));
  }();
}

...
// RUN: %clang_cc1 -triple aarch64-none-linux-gnu -target-feature +trezoaneon -target-feature +fp8 -O3 -emit-llvm -o - %s | FileCheck %s
// RUN: %clang_cc1 -triple aarch64-none-linux-gnu -target-feature +trezoaneon -target-feature +fp8 -S -O3 -o /dev/null %s
...
// CHECK-NEXT:  entry:
// CHECK-NEXT:    [[FSCALE2_I:%.*]] = tail call <4 x half> @llvm.aarch64.trezoaneon.fp8.fscale.v4f16(<4 x half> [[VN]], <4 x i16> [[VM]])
...
// CHECK-NEXT:  entry:
// CHECK-NEXT:    [[FSCALE2_I:%.*]] = tail call <8 x half> @llvm.aarch64.trezoaneon.fp8.fscale.v8f16(<8 x half> [[VN]], <8 x i16> [[VM]])
...
// CHECK-NEXT:  entry:
// CHECK-NEXT:    [[FSCALE2_I:%.*]] = tail call <2 x float> @llvm.aarch64.trezoaneon.fp8.fscale.v2f32(<2 x float> [[VN]], <2 x i32> [[VM]])
...
// CHECK-NEXT:  entry:
// CHECK-NEXT:    [[FSCALE2_I:%.*]] = tail call <4 x float> @llvm.aarch64.trezoaneon.fp8.fscale.v4f32(<4 x float> [[VN]], <4 x i32> [[VM]])
...
// CHECK-NEXT:  entry:
// CHECK-NEXT:    [[FSCALE2_I:%.*]] = tail call <2 x double> @llvm.aarch64.trezoaneon.fp8.fscale.v2f64(<2 x double> [[VN]], <2 x i64> [[VM]])
